...
func _process(_delta):
	...
	if $SM.current_target:
		...
		if target_vector.x < 0:
			$MinotaurArt.flip_h = false
		elif target_vector.x > 0:
			$MinotaurArt.flip_h = true
...
func take_hit(hit_position, damage, hitstun=50):
	...
	if $UI/HealthBar.value <= 0:
		...
		selected = false
		controllable = false
	elif $UI/HealthBar.value <= unitHealth / 2:
		rage()
	else:
...
func rage():
	...
	runSpeed = runSpeed * rageMult
	weaponRange = weaponRange - 5
